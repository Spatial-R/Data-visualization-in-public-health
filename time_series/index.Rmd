...
As with all of R, the ability to easily chart time series is the result of an iterative progression driven by the collaboration of an extremely dedicated group of open source volunteers.  With the release of [`rCharts`](http://ramnathv.github.io/rCharts),[`dygraph`](http://rstudio.github.io/dygraphs/) I thought it would be interesting to document the timeline of this progression.  
...
h1 <- hPlot(
  aqi~ date,group="city",data = AQI.HSW,type = "line"
)

h1$xAxis(type = "datetime");h1$chart(zoomType = "x")

h1$plotOptions(line = list(marker = list(enabled = F)))
...
AQI.RC$date<-NULL; names(AQI.RC)<-c("Hangzhou","Shenzheng")
dygraph(AQI.RC) %>%
