...
polarHistogram<-function(
  ...
  readableJustification<-function(x){
    angle<-x*(-360/totalLength)-alphaStart*180/pi+90
    ifelse(sign(cos(angle*pi/180))+sign(sin(angle*pi/180))==-2,1.0,-0.21)
  }
...
#   # empty background and remove guide lines, ticks and labels
  ...
  # nice colour scale
  p<-p+scale_fill_brewer(palette="Set2",type="seq",
                         guide = guide_legend(title = "",direction="horizontal",
